...
pub fn detect_unchecked_return(function: &FunctionInfo, symbol_pool: &SymbolPool, idx: usize, cm: &CompiledModule) -> bool {
    ...
    if be_call_funcs.is_empty() {
        ...
        for fun in be_call_funcs.iter() {
            println!("unckecked return detection info:\nfunction **:({}) has return values but do not be used in {}", symbol_pool.string(*fun), cm.identifier_at(cm.function_handle_at(cm.function_defs[idx].function).name));
        }
        return true;
    }
}
